...
Within scoped access control, there is always at least two scopes associated with a given policy. These two scopes can be thought
of as the scope of *origin* and the scope of *effect*, or alternatively the scope of *provenance* and the scope of *obligation*.
These are the scopes that the policy originates *from*, and the scope that the policy is intended to apply *to*, respectively.

- *Scope of Origin*: This is the scope of the resource that the policy is defined and/or assigned by. This scope informs us of
the authority from which the policy originates. It is, in effect, a representation of the *provenance* of the policy. A higher
level scope of origin represents a policy that is intended to express the intent of higher level admins, and therefore carries
greater weight all else being equal. In particular, it should not be possible for a policy with a higher/ancestral scope of origin
...
The scope of effect *must* be equivalent to or descendant from the scope of origin. This ensures that policies cannot "reach up"
...
Understanding the difference between these two kinds of scope is essential for understanding why scoped role evaluation order is
defined the way that it is. They explain not only our chosen ordering, but why scoped access subtly differs from other similar systems
with similar properties. Typically, access control systems tend tend to gravitate towards a "most specific rule wins" model, where the policy
with the narrowest defined target takes precedence. Hierarchical systems often unilaterally apply the most ancestral policy first.
We are going to use a mix of both. The *Scope of Origin* uses ancestral preference to ensure hierarchical isolation, while the *Scope of Effect*
uses specificity preference to ensure that administrative intent is easily expressible.

### Role Evaluation Order

Role evaluation order should be deterministic, and respect hierarchical isolation principles. In order to achieve this,
we will be using a three-tiered approach to ordering of scoped role evaluation (discussed below). In short, the three
determinants, in order, are: scope of the originating assignment resource (*Scope of Origin*), scope of the role's effective
privilege assignment (*Scope of Effect*), and lexicographic order of the role name.

The primary determinant of role evaluation order is the scope of the assignment *resource* that granted the role. Scoped
role assignments are resources in their own right, and have their own scope attribute. This scope is what we consider the
*Scope of Origin* for a policy state represented by a given role being assigned to a given user. Ordering by this scope is
performed from top to bottom, with more ancestral assignments being evaluated first. The scope of the assignment
resource is what determines who is able to create/edit/delete the assignment, with higher level admins creating assignment
resources at parent scopes, and lower level admins creating assignment resources at child scopes. This means that roles assigned
via assignment resources at parent scopes represent the *intent* of higher level admins and/or express a policy of greater
authority. By evaluating roles assigned by parent scope assignment resources first, we ensure that the intent of higher level
admins takes precedence, preserving hierarchical isolation.

The secondary determinant of role evaluation order is the scope at which the role's privileges are actually active for the user. This is
the *Scope of Effect* for the policy state represented by a given role being assigned to a given user. This secondary ordering uses
specificity preference, with descendant scopes being evaluated before ancestral scopes. This ensures that more specific
policies take precedence over more general policies, allowing admins to express specialized intent at narrower scopes.
An assigned role whose privileges are in effect only for `/staging/west` will be evaluated before a role whose privileges
are in effect for all of `/staging`, and so-on. This diverges from the more common ancestral preference model, but since
we are already enforcing hierarchical isolation via the primary ordering, we are free to let admins within a given scope express
more specific intent without worrying about violating isolation principles.

Finally, we need to order role evaluation in cases where the scope of the *Scope of Origin* and *Scope of Effect* are the same.
In this case, we will fall back to lexicographic order of the role name. This is somewhat arbitrary, but it is simple, deterministic,
...
Scope         Name                                 User  Assigns                                                        
------------- ------------------------------------ ----- -------------------------------------------------------------- 
/staging      ca2cbaf5-d335-4d1f-a8d0-18601104d371 alice staging-owner -> /staging/west, staging-auditor -> /staging 
/staging/west 76edb675-bb47-4582-8882-6368661209ad alice staging-west-user -> /staging/west
/staging/west b1597673-aaf4-4ee5-93fc-2583ebaeaac5 alice staging-west-dev  -> /staging/west                            
... 
```

In this case, when `alice` attempts to access a resource in `/staging/west`, the order of role evaluation will be
`staging-owner`, `staging-auditor`, `staging-west-dev`, `staging-west-user`. All assignments originating from `/staging`
are  evaluated before any originating from `/staging/west`. Within the set of roles originating from `/staging`, `staging-owner` is
evaluated before `staging-auditor` due to having a more specific scope of effect. Then evaluation proceeds to the roles originating
from `/staging/west`. These roles have the same scope of effect, and so lexicographic order of the role name is used to disambiguate.
...
strict timeout, the resource-aware layer cen enforce that as needed. This approach works well for many usecases,
...
- We can introduce simple flows to allow admins to "test drive" individual roles without needing to worry about their
  existing role set introducing side-effects.
