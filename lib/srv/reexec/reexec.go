...
func RunCommand() (errw io.Writer, code int, err error) {
	...
	if cmdfd == nil {
		return errorWriter, RemoteCommandFailure, trace.BadParameter("command pipe not found")
	}
	contfd := os.NewFile(ContinueFile, fdName(ContinueFile))
	if contfd == nil {
		return errorWriter, RemoteCommandFailure, trace.BadParameter("continue pipe not found")
	}
	readyfd := os.NewFile(ReadyFile, fdName(ReadyFile))
	if readyfd == nil {
		return errorWriter, RemoteCommandFailure, trace.BadParameter("ready pipe not found")
	}
	...
	if terminatefd == nil {
		return errorWriter, RemoteCommandFailure, trace.BadParameter("terminate pipe not found")
	}

	// Read in the command payload.
	var c ExecCommand
	if err := json.NewDecoder(cmdfd).Decode(&c); err != nil {
		return io.Discard, RemoteCommandFailure, trace.Wrap(err)
	}
	...
	if c.Terminal {
		...
		if tty == nil {
			return errorWriter, RemoteCommandFailure, trace.BadParameter("tty not found")
		}
		errorWriter = tty
	}
	...
	if c.PAMConfig != nil {
		...
		if err != nil {
			return errorWriter, RemoteCommandFailure, trace.Wrap(err)
		}
	...
	if c.SetSELinuxContext {
		...
		if err != nil {
			return errorWriter, RemoteCommandFailure, trace.Wrap(err, "failed to get SELinux context of login user")
		}
		...
		if err := ocselinux.SetExecLabel(seContext); err != nil {
			return errorWriter, RemoteCommandFailure, trace.Wrap(err, "failed to set SELinux context")
		}
	}
...
func RunNetworking() (errw io.Writer, code int, err error) {
	...
	localUser, err := user.Lookup(c.Login)
	if err != nil {
		return errorWriter, RemoteCommandFailure, trace.NotFound("%s", err)
	}
	...
	if os.Getuid() != int(cred.Uid) || os.Getgid() != int(cred.Gid) {
		if !cred.NoSetGroups {
			...
			if err := unix.Setgroups(groups); err != nil {
				return errorWriter, RemoteCommandFailure, trace.Wrap(err, "failed to set groups for networking process")
			}
		}
		if err := unix.Setgid(int(cred.Gid)); err != nil {
			return errorWriter, RemoteCommandFailure, trace.Wrap(err, "failed to set gid for networking process")
		}
		if err := unix.Setuid(int(cred.Uid)); err != nil {
			return errorWriter, RemoteCommandFailure, trace.Wrap(err, "failed to set uid for networking process")
		}
	}
	...
	for _, kv := range pamEnvironment {
		...
		if !ok {
			return errorWriter, RemoteCommandFailure, trace.BadParameter("bad environment variable from PAM, expected format \"key=value\" but got %q", kv)
		}
	...
	if err := os.Chdir(workingDir); err != nil {
		return errorWriter, RemoteCommandFailure, trace.Wrap(err, "failed to set working directory for networking process: %s", workingDir)
	}

	// Build request listener from first extra file that was passed to command.
	ffd := os.NewFile(FirstExtraFile, "listener")
	if ffd == nil {
		return errorWriter, RemoteCommandFailure, trace.BadParameter("missing socket fd")
	}
	...
	for {
		...
		if err != nil {
			return errorWriter, RemoteCommandSuccess, nil
		}
...
func RunAndExit(commandType string) {
	...
	case ParkSubCommand:
		w, code, err = runPark()
	default:
		w, code, err = os.Stderr, RemoteCommandFailure, fmt.Errorf("unknown command type: %v", commandType)
	}
	if err != nil {
