...
func (p *ResourceMonitor[T]) getCurrentResources(ctx context.Context) error {
	...
	// them based on whether they have been seen before.
	resources := map[string]T{}
...
func (p *ResourceMonitor[T]) handleEvent(ctx context.Context, event types.Event) {
	...
	case types.OpPut:
		...
		switch {
		// this is an existing resource that should be updated
		case ok && matches:
			p.resources[key] = t
		// this is a new resource that should be created
		case !ok && matches:
			p.resources[key] = t
		// this is an existing resource that should no longer be tracked
		case ok && !matches:
		}
		p.mu.Unlock()
